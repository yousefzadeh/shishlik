...
- No layered data architecture; views built organically without clear separation of concerns
  ...
  - TenantId not pushed down to all view layers
...
- [ ] Pick something already implemented on Synapse (e.g. QBA Question Answer)
...
- Near-zero for non-busy regions (JP, SG, CA, QA)
- Manageable for busy regions (AU, UK, US)
