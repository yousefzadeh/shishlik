...
, Assigneelist as (
select distinct ActionId, string_agg(coalesce(AssigneeFilter, 'Unassigned'), ', ') AssigneeList
from Responsibility r
group by ActionId
)
, final as(
select max.*, al.AssigneeList, max(max.no_count) over (PARTITION by max.RootStatementId) as max_no_count
from max
left join
    AssigneeList al on al.ActionId = max.ActionId
)
