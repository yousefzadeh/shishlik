...
{# 
DOC START
  - name: vwThirdPartyCustomFieldValue
    description: |
    ...
    columns:
      ...
      - name: CustomField_Value
        description: |
            Value assigned (Converted to Text as per APP UI) to the Custom Field For the Entity (ThirdParty_Id) at the Tenant (Tenant_Id)
            - Dropdown - One Value
            - Matrix - One Value
...
with 
{#- 
    ThirdParty to Custom Field Values assignments
...
entity_custom_selected as (
	-- Third Party (TenantVendor)
    select 
    NULL Tenant_Id,
	ThirdPartyData_TenantVendorId Entity_Id, -- ThirdParty_Id
    ThirdPartyData_ThirdPartyAttributesId SelectedOption_Id,
    NULL CustomField_Id, -- for user and organization unit
    NULL UserId,
    NULL OrganizationUnitId
    from {{ ref("vwThirdPartyData") }}
),
entity_custom_user_defined as (
    -- Third Party (TenantVendor)
    select 
	ThirdPartyFreeTextControlData_TenantVendorId Entity_Id, -- ThirdParty_Id
    ThirdPartyFreeTextControlData_ThirdPartyControlId CustomField_Id,
    ThirdPartyFreeTextControlData_TextData TextValue,
    ThirdPartyFreeTextControlData_NumberValue NumberValue,
    ThirdPartyFreeTextControlData_CustomDateValue DateValue
    from {{ ref("vwThirdPartyFreeTextControlData") }}
),
{#- ------------------------------------------------------------------------------------ #}
{#- 
    Custom Fields
...
custom_field_defined as (
	...
	where CustomField_EntityTypeId = 0 -- Third Party (TenantVendor)
),
custom_field as (
...
final as (
	...
	cast(u.CustomField_DateValue as date) as CustomField_DateValue,
