...
DOC END        
RiskAssessmentCustomAttributeData  RiskAssessmentThirdPartyControlFreeText
#}

with 
{#- 
    Risk Assessment to Custom Field Values assignments
    - Risk Assessment to Custom Field Values assignments for selection from options  
    - does not have User Type (User and Organization Unit) 
    - Risk Assessment to Custom Field Values assignments for user defined values
    - does not have Number Type
#}
entity_custom_selected as (
    select 
    RiskAssessmentCustomAttributeData_TenantId Tenant_Id,
	RiskAssessmentCustomAttributeData_RiskAssessmentId Entity_Id, 
    RiskAssessmentCustomAttributeData_ThirdPartyAttributesId SelectedOption_Id,
    NULL CustomField_Id, -- for user and organization unit
    NULL UserId,
    NULL OrganizationUnitId
    from {{ ref("vwRiskAssessmentCustomAttributeData") }}
),
entity_custom_user_defined as (
    -- Risk Assessment has no Number Value
    select 
	RiskAssessmentThirdPartyControlFreeText_TenantId Tenant_Id,
	RiskAssessmentThirdPartyControlFreeText_RiskAssessmentId Entity_Id, -- Risk_Id
    RiskAssessmentThirdPartyControlFreeText_ThirdPartyControlId CustomField_Id,
    RiskAssessmentThirdPartyControlFreeText_TextData TextValue,
    NULL NumberValue,
    RiskAssessmentThirdPartyControlFreeText_CustomDateValue DateValue
    from {{ ref("vwRiskAssessmentThirdPartyControlFreeText") }}
),
{#- ------------------------------------------------------------------------------------ #}
{#- 
    Custom Fields
...
custom_field_defined as (
	...
	where CustomField_EntityTypeId = 4 -- Risk Assessment
),
custom_field as (
