...
{# 
DOC START
  - name: vwIssueCustomFieldValue
    description: |
      This view shows the Issues with Custom Fields and Values for the Tenant.
    ...
    columns:
      ...
      - name: Issue_Id
        description: ID of Issue as defined in Issues table
      - name: CustomField_Id
      ...
      - name: CustomField_Value
        description: |
            Value assigned (Converted to Text as per APP UI) to the Custom Field For the Entity (Issue_Id) at the Tenant (Tenant_Id)
            - Dropdown - One Value
            - Matrix - One Value
...
DOC END        
IssueCustomAttributeData  IssueFreeTextControlData
#}

with 
{#- 
    Issues to Custom Field Values assignments
    - Issues to Custom Field Values assignments for selection from options  
    - does not have User Type (User and Organization Unit) 
    - Issues to Custom Field Values assignments for user defined values
    - does not have Number Type
#}
entity_custom_selected as (
	-- Issue
    select 
    icad.IssueCustomAttributeData_TenantId Tenant_Id,
	icad.IssueCustomAttributeData_IssueId Entity_Id, 
    icad.IssueCustomAttributeData_ThirdPartyAttributesId SelectedOption_Id,
    NULL CustomField_Id, -- for user and organization unit
    NULL UserId,
    NULL OrganizationUnitId
    from {{ ref("vwIssues") }} i 
	join {{ ref("vwIssueCustomAttributeData") }} icad on icad.IssueCustomAttributeData_IssueId = i.Issues_Id
),
entity_custom_user_defined as (
    -- Issue - IssueFreeTextControlData has no TenantId
    select 
	ift.IssueFreeTextControlData_IssueId Entity_Id, -- Issue_Id
    ift.IssueFreeTextControlData_ThirdPartyControlId CustomField_Id,
    ift.IssueFreeTextControlData_TextData TextValue,
    ift.IssueFreeTextControlData_NumberValue NumberValue,
    ift.IssueFreeTextControlData_CustomDateValue DateValue
	from {{ ref("vwIssues") }} i 
	join {{ ref("vwIssueFreeTextControlData") }} ift on ift.IssueFreeTextControlData_IssueId = i.Issues_Id
),
{#- ------------------------------------------------------------------------------------ #}
{#- 
    Custom Fields
...
custom_field_defined as (
	...
	where CustomField_EntityTypeId = 6 -- Issues
),
custom_field as (
...
{#- ---------------------------------------------------------------------------
    Custom Field Values
    Custom Field Values are defined at the Tenant (Tenant_Id) for the Entity (Issue_Id) for the Custom Field (CustomField_Id)
...
case_user as ( 
	...
	inner join entity_custom_selected cfs  -- Custom Field Selected 
		...
		-- no TenantId in entity_custom_selected
...
case_text as (
	...
	inner join entity_custom_user_defined cfs  -- Custom Field Selected 
		on cfs.CustomField_Id = cf.CustomField_Id 
		-- No TenantId in entity_custom_user_defined
...
case_date as (
	...
	cf.CustomField_Id,
	cf.CustomField_Name,
	NULL CustomField_Value,
	NULL CustomField_TextValue,
	NULL CustomField_NumberValue,
	cfs.DateValue as CustomField_DateValue,
	{# FORMAT(cfs.DateValue, 'MMM d, yyyy') CustomField_DateValue, #}
	cf.CustomField_Type,
	NULL CustomField_ValueCode
	from custom_field cf                       -- Custom Field Options
	inner join entity_custom_user_defined cfs  -- Custom Field Selected 
...
final as (
	...
	select
	-- Grain at One row per Tenant per IssueId, per CustomField, per CustomFieldValue
	cf.RoleType,
	cf.Tenant_Id,
	u.Entity_Id Issue_Id,
	cf.CustomField_Id,
