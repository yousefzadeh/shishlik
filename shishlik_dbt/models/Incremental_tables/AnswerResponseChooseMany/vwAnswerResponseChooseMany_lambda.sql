...
with
    batch as (
        select
            ...
            AnswerResponse_PK
        from {{ ref("AnswerResponseChooseMany_batch") }}
    ),
    -- ----- CUSTOM SOURCE
    max_batch_date as (
        select max(AnswerResponse_UpdateTime) as Max_UpdateTime from {{ ref("AnswerResponseChooseMany_batch") }}
    ),
    QUESTION as (
        select *
        ...
        where
            Question_Type = 3
            and Question_IsDeleted = 0
            and Question_UpdateTime > (select Max_UpdateTime from max_batch_date)
        union all
        select *
        from {{ ref("vwQuestionAll") }}
        where
    ...
    answer_raw as (
        ...
        where
            IsDeleted = 0 and coalesce(LastModificationTime, CreationTime) > (select Max_UpdateTime from max_batch_date)
    ),
    ANSWER as (
        select
            [ID] as [Answer_ID],
    ...
    stream as (
        select
            ...
            AnswerResponse_PK
        from source
    -- where AnswerResponse_UpdateTime > ( select max(batch.AnswerResponse_UpdateTime) from batch )
    )
select *
from batch
union all
