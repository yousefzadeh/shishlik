...
with by_month as (
    SELECT DISTINCT
      --  VWABPUSERLOGINATTEMPTS.AbpUserLoginAttempts_CreationTimeMonth CreationTimeMonth,
       DATEADD(MONTH, DATEDIFF(MONTH, 0, vwAbpUserLoginAttempts.AbpUserLoginAttempts_CreationTime), 0) CreationMonthDate, 
       VWABPTENANTS.AbpTenants_Name,
       VWABPTENANTS.AbpTenants_Id,
       ae.AbpEditions_DisplayName
    FROM {{ ref("vwAbpTenants") }} AS VWABPTENANTS
    LEFT OUTER JOIN {{ ref("vwAbpUserLoginAttempts") }} AS VWABPUSERLOGINATTEMPTS
    ON (
       VWABPTENANTS.AbpTenants_Id = VWABPUSERLOGINATTEMPTS.AbpUserLoginAttempts_TenantId
    )
    AND (
       VWABPUSERLOGINATTEMPTS.AbpUserLoginAttempts_Result = 1
    )
