...
with final as (
    SELECT DISTINCT
        ...
        ass.Assessment_AssessmentVersionName ResponseVersion,
        ass.Assessment_QuestionType QuestionType,
        MAX(GREATEST(cast(asst.Assessment_UpdateTime as datetime2), cast(ass.Assessment_UpdateTime as datetime2))) OVER (PARTITION BY asst.Assessment_TenantId,asst.Assessment_Name,ass.Assessment_Name,ass.Assessment_AssessmentVersionName,ass.Assessment_QuestionType) AS Filter_UpdateTime
    FROM
