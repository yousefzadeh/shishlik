...
with
    ...
    unlinked_auth_question as (
        ...
        inner join map on auth_ass.Provision_Id = map.SourceAuthorityProvisionId and auth_ass.Tenant_Id = map.TenantId
        where
            not exists (
                select 1
                from {{ ref("fact_provision_question_relation") }} auth_question
                where
    ...
    linked_fact as (
        ...
        select
            ...
            auth.Name + case
                when auth.IsDeleted = 1 then '(Deleted)' when auth.IsArchived = 1 then '(Archived)' else ''
            ...
            target_auth.Name + case
                when auth.IsDeleted = 1 then '(Deleted)' when auth.IsArchived = 1 then '(Archived)' else ''
            end TargetAuthority_Name,  -- target authority
        ...
        inner hash join
            ...
            inner hash
        join
            {{ ref("fact_provision_custom_field_value") }} custom  -- target provision
            on auth_question.TargetAuthorityProvisionId = custom.Provision_Id
            and auth_question.Tenant_Id = custom.Tenant_Id
        inner join {{ source("assessment_models", "AuthorityProvision") }} prov on auth_question.Provision_Id = prov.Id
        inner join {{ source("assessment_models", "Authority") }} auth on auth.Id = prov.AuthorityId
        inner join
        ...
        inner join {{ ref("dim_assessment") }} ass on auth_question.Assessment_Id = ass.Assessment_Id
    ),
    unlinked_fact as (
