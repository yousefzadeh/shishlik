...
with
    ...
    flat_controls_detail as (
        ...
        left join {{ ref("vwAuthority") }} va on a.Authority_Id = va.Authority_Id
    ),
    ...
    flat_mapped_controlset_detail as (
        ...
        left join {{ ref("vwAuthority") }} a on a.Authority_Id = m.Authority_Id
    ),
    uni_detail as (
        select *
...
select
    ...
    Policy_Name,
    ClassName,
    count(distinct ClassId) all_count,
    count(distinct MappedClassId) mapped_count,
    string_agg(MappedClassName, ',') mapped_list
from uni_detail
-- where Tenant_Id = 3 and Authority_Id = 10
